      ...
      setIsLoadingCep(true);
       try {
         ...
         if (!data.erro) {
            ...
            // Optional: Focus on number field programmatically could be added here
         }
       } catch (err) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-slate-700 mb-1">Nome da Empresa (Razão Social/Fantasia)</label>
                    <input required className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.companyName} onChange={e => setClientForm({...clientForm, companyName: e.target.value})} />
                  </div>
                  
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-slate-700 mb-1">Nome do Responsável</label>
                    <input required className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.name} onChange={e => setClientForm({...clientForm, name: e.target.value})} />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Telefone 1 (Obrigatório)</label>
                    <input required placeholder="(00) 00000-0000" className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.phone} onChange={e => setClientForm({...clientForm, phone: e.target.value})} />
                  </div>
                  
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Telefone 2 (Opcional)</label>
                    <input placeholder="(00) 00000-0000" className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.phone2} onChange={e => setClientForm({...clientForm, phone2: e.target.value})} />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">CPF / CNPJ</label>
                    <input className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.cpfCnpj} onChange={e => setClientForm({...clientForm, cpfCnpj: e.target.value})} />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-1">Inscrição Estadual</label>
                    <input className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.stateRegistration} onChange={e => setClientForm({...clientForm, stateRegistration: e.target.value})} />
                  </div>

                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
                    <input type="email" className="w-full border p-2.5 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" value={clientForm.email} onChange={e => setClientForm({...clientForm, email: e.target.value})} />
                  </div>
                     ...
                     <div className="grid grid-cols-6 gap-3">
                        <div className="col-span-2">
                           ...
                           <div className="relative">
                              <input 
                                className="w-full border p-2 rounded-lg focus:ring-2 focus:ring-indigo-200 outline-none" 
                                placeholder="00000-000"
                                value={clientForm.zipCode || ''} 
                                onChange={e => setClientForm({...clientForm, zipCode: e.target.value})}
                                onBlur={checkCep}
                              />
                              {isLoadingCep && <div className="absolute right-2 top-2.5 text-xs text-indigo-500 animate-pulse">...</div>}
                           </div>
                        </div>
                         <div className="col-span-3">
                           <label className="block text-xs font-medium text-slate-600 mb-1">Cidade</label>
                           <input className="w-full border p-2 rounded-lg bg-slate-100" value={clientForm.city} onChange={e => setClientForm({...clientForm, city: e.target.value})} />
                        </div>
                        <div className="col-span-1">
                           <label className="block text-xs font-medium text-slate-600 mb-1">UF</label>
                           <input className="w-full border p-2 rounded-lg bg-slate-100" maxLength={2} value={clientForm.state || ''} onChange={e => setClientForm({...clientForm, state: e.target.value})} />
                        </div>

                        <div className="col-span-5">
                           <label className="block text-xs font-medium text-slate-600 mb-1">Logradouro (Rua, Av...)</label>
                           <input className="w-full border p-2 rounded-lg" value={clientForm.address} onChange={e => setClientForm({...clientForm, address: e.target.value})} />
                        </div>
                        <div className="col-span-1">
                           <label className="block text-xs font-medium text-slate-600 mb-1">Número</label>
                           <input className="w-full border p-2 rounded-lg" value={clientForm.number || ''} onChange={e => setClientForm({...clientForm, number: e.target.value})} />
                        </div>
                        
                        <div className="col-span-3">
                           <label className="block text-xs font-medium text-slate-600 mb-1">Bairro</label>
                           <input className="w-full border p-2 rounded-lg" value={clientForm.neighborhood || ''} onChange={e => setClientForm({...clientForm, neighborhood: e.target.value})} />
                        </div>
                     </div>
                  </div>
              </div>
